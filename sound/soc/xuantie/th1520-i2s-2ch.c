...
static inline void th1520_snd_txctrl(struct th1520_i2s_priv *chip, bool on)
{
	...
	if (on) {
		...
		if (!chip->mclk_keepon) {
			...
			* the DMA configuration for RX and TX is completely
			* disabled.
			*/
...
static inline void th1520_snd_rxctrl(struct th1520_i2s_priv *chip, bool on)
{
	...
	if (on) {
		...
		if (!chip->mclk_keepon) {
			/*
			* The enablement of I2S can onlybe truned off when
			* the DMA confgiguratio for RX and TX is completely
			* disabled.
			*/
			if (((DMACR_TDMAE_MSK | DMACR_RDMAE_MSK) & dma_en) == 0)
			{
	...
	if (strcmp(priv->name, AP_I2S)) {
		...
		if (!strcmp(priv->name, AUDIO_I2S0))
			regmap_update_bits(priv->audio_cpr_regmap,
					   CPR_IP_RST_REG,
					   CPR_I2S0_SRST_N_SEL_MSK,
					   CPR_I2S0_SRST_N_SEL(0));
		else if (!strcmp(priv->name, AUDIO_I2S1))
			regmap_update_bits(priv->audio_cpr_regmap,
					   CPR_IP_RST_REG,
					   CPR_I2S1_SRST_N_SEL_MSK,
					   CPR_I2S1_SRST_N_SEL(0));
		else if (!strcmp(priv->name, AUDIO_I2S2))
			regmap_update_bits(priv->audio_cpr_regmap,
					   CPR_IP_RST_REG,
					   CPR_I2S2_SRST_N_SEL_MSK,
					   CPR_I2S2_SRST_N_SEL(0));
	}
	...
	if (strcmp(priv->name, AP_I2S)) {
		if (!strcmp(priv->name, AUDIO_I2S0))
			regmap_update_bits(priv->audio_cpr_regmap,
					   CPR_IP_RST_REG,
					   CPR_I2S0_SRST_N_SEL_MSK,
					   CPR_I2S0_SRST_N_SEL(1));
		else if (!strcmp(priv->name, AUDIO_I2S1))
			regmap_update_bits(priv->audio_cpr_regmap,
					   CPR_IP_RST_REG,
					   CPR_I2S1_SRST_N_SEL_MSK,
					   CPR_I2S1_SRST_N_SEL(1));
		else if (!strcmp(priv->name, AUDIO_I2S2))
			regmap_update_bits(priv->audio_cpr_regmap,
					   CPR_IP_RST_REG,
					   CPR_I2S2_SRST_N_SEL_MSK,
					   CPR_I2S2_SRST_N_SEL(1));
...
static int th1520_audio_i2s_probe(struct platform_device *pdev)
{
	...
	sprop = of_get_property(np, "th1520,mode", NULL);
	if (sprop) {
	...
	sprop = of_get_property(np, "th1520,sel", NULL);
	if (sprop) {
		strcpy(i2s_priv->name, sprop);
	}

	iprop = of_get_property(np, "th1520,dma_maxburst", NULL);
	if (iprop)
	...
	iprop = of_get_property(np, "th1520,mclk_keepon", NULL);
	if (iprop)
	...
	if (!strcmp(i2s_priv->name, AUDIO_I2S0) ||
		!strcmp(i2s_priv->name, AUDIO_I2S1) ||
		!strcmp(i2s_priv->name, AUDIO_I2S2)) {
		i2s_priv->audio_pin_regmap =
			syscon_regmap_lookup_by_phandle(np, "audio-pin-regmap");
		if (IS_ERR(i2s_priv->audio_pin_regmap)) {
			dev_err(&pdev->dev,
				"can't find regmap for audio system register\n");
		} else
			th1520_audio_pinctrl(&pdev->dev);
	...
	} else if (!strcmp(i2s_priv->name, AUDIO_I2S0) ||
			!strcmp(i2s_priv->name, AUDIO_I2S1) || 
			!strcmp(i2s_priv->name, AUDIO_I2S2)) {
		i2s_priv->dma_params_tx.addr = res->start + I2S_DR;
